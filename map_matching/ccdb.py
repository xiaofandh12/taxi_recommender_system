...
taxi_path_name_prefix = 'taxi_paths_occupied_crusing_'
taxi_tracks_name_prefix = 'trajectories_view_'

def create_path_occupied_crusing_table(tbname_suffix,method):
    conn = pg.connect(database = 'beijing_taxi_201211',user = 'postgres',password = 'zq600991')
    cursor = conn.cursor()
    tbname_paths_occupied_crusing = taxi_path_name_prefix + method +'_' + tbname_suffix
    sql = r"CREATE TABLE IF NOT EXISTS " + tbname_paths_occupied_crusing + r"""(
	tid int null,
...
def create_path_occupied_crusing_table_attr(tbname_suffix,method):
    conn = pg.connect(database = 'beijing_taxi_201211', user = 'postgres', password = 'zq600991')
    cursor = conn.cursor()
    tbname_paths_occupied_crusing = taxi_path_name_prefix + method + '_' +  tbname_suffix
    tbname_paths_occupied_crusing_attr = taxi_path_name_prefix + method + '_' +  tbname_suffix + '_attr'
    sql = r"CREATE TABLE IF NOT EXISTS " + tbname_paths_occupied_crusing_attr + r"""(
	tid int null,
...
def tbname_of_method(tbname_suffix,method):
    if method in ['bn','st','iv','ut','uti']:
        return taxi_path_name_prefix + method + '_' + tbname_suffix
    else:
        return None

def tbname_of_purpose(tbname_suffix,purpose):
    if purpose in ['dp','mm']:
        return taxi_tracks_name_prefix + tbname_suffix
    else:
        return None
...
def new_path_writer_for_method(tbname_suffix,method):
    tbname = tbname_of_method(tbname_suffix,method)
    if tbname is not None:
        return PathWriter(tbname=tbname)
    else:
        return None

def new_path_attr_writer(tbname):
        return PathAttrWriter(tbname=tbname)

def new_path_attr_writer_for_method(tbname_suffix,method):
    tbname = tbname_of_method(tbname_suffix,method)
    if tbname is not None:
        tbname = tbname + "_attr"
...
def new_track_reader_for_purpose(tbname_suffix,purpose,limit=10000, offset=0):
    print purpose
    tbname = tbname_of_purpose(tbname_suffix,purpose)
    if tbname is not None:
...
class WayDB(DB):
    def __init__(self, tbname='ways'):
        super(WayDB, self).__init__(database='beijing_taxi_201211')
        self.tbname = tbname
...
class WayAttrDB(DB):
    def __init__(self, tbname):
        super(WayAttrDB, self).__init__(database='beijing_taxi_201211')
        self.tbname = tbname
...
class TrackReader(DB):
    def __init__(self, tbname=taxi_tracks_name_prefix + "20121101", limit = 10000, offset = 0):#？TrackReader(tbname=tbname, limit=limit, offset=offset)
    ...
    def query_more(self):#对track按照id进行排序，再从offset处选出limit条track
        sql = "select id, cuid_count, st_astext(track_geom) as geom, track_desc from " + self.tbname + " order by id limit %s offset %s"
        self.cursor.execute(sql, (str(self.limit), str(self.offset)))
        self.offset = self.offset + self.limit
        self.queried_num = self.queried_num + self.limit
    ...
    def fetch_one(self):
        ...
        self.fetched_num = self.fetched_num + 1
        return ct.new_track_from_db(row['id'], row['cuid_count'], row['geom'], row['track_desc'])
