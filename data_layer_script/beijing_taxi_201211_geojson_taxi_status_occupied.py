...
class Geojson:
	...
	def cuid_end_callback(self,cuid):
		...
		self.write_to_file(proc_output_dir +'taxi_status_occupied/' + prefix + '_' + str(CUID_count) + '_taxi_status_occupied' + '.json')
	def line_callback(self,line):
		if line != '':
			...
			elif (self.taxi_status != attrs['taxi_status']) and (self.pre_taxi_status == '1'):
				self.coords.append(coord)
				self.descs.append({'t':time_tx,'h':head_tx,'s':speed_tx,'taxi_status':taxi_status_tx})
...
for tbname in tbnames:
	...
	for CUID_count in range(1,1001):
		...
		print proc_output_dir + 'taxi_status_occupied/' + prefix +'_' + str(CUID_count) + '_taxi_status_occupied' + '.json' , 'completed'
